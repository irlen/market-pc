...
class MapDistrict extends Component{
  state = {
    curProps:{},
    spinning: false,
    autoHeight: 0
       ...
       bdary.get(district, (rs) => {
         ...
         for (let i = 0; i < count; i++) {
           const ply = new BMap.Polygon(rs.boundaries[i], {
             strokeWeight: 2,
             strokeColor: '#0033FF', //行政区域分界线颜色
             strokeOpacity: 1,
             fillColor: '#0099FF', //行政区域填充背景颜色颜色
             fillOpacity: 1
            });
           map.addOverlay(ply);
         }
  ...
  //  map.addControl(new BMap.NavigationControl(opts)) //缩放平移控件
    ...
    if(centerPoint && centerPoint.length){
      map.centerAndZoom(new BMap.Point(centerPoint[0], centerPoint[1]), initScale?initScale:11)
    }
    //根据城市名定位
    if(centerCity){
    ...
    if(overlayData){
      ...
      ComplexCustomOverlay.prototype.initialize = function(map){
        ...
        span.style.margin = " -20px 0 0 30px"
        ...
        div.onmouseover = function(){
          ...
          const fatherSpan = this.getElementsByTagName("span")[0]
          $(fatherSpan).append(that._overText)
      ...
      overlayData.map(item=>{
        if(item.type !== 'enterprise'){
          ...
          let mouseoverText = ''
          if(item.type === 'park' || item.type === 'build'){
            const name = item.name
            const enterpriseCount = item.enterprise
            let primaryIndusDom= ''
            const primaryIndus = item.prime_industry
            let parkFunctionDom = ''
            const parkFunction = item.park_function

            if(primaryIndus){
              const primaryArr = primaryIndus.split(',')
              primaryArr.map(item=>{
                primaryIndusDom += '<span style="display:block;color:#00FF33;">'+item+'</span>'
              })
            }
            if(parkFunction){
              const primaryArr = parkFunction.split(',')
              primaryArr.map(item=>{
                parkFunctionDom += '<span style="display:block;color:#00FF33;">'+item+'</span>'
              })
            }
            mouseoverText += '<div style="color:#00FF33;font-weight: bold;">'+name+'</div>'
            mouseoverText += '<div style="width:100%;display:flex;"><div style="flex:0 0 60px;">企业数量:</div><div style="color:#00FF33;flex:1 1 auto;"> '+enterpriseCount+' </div></div>'
            mouseoverText += '<div style="width:100%;display:flex;"><div style="flex:0 0 60px;">主导产业:</div><div style="color:#00FF33;flex:1 1 auto;"> '+primaryIndusDom+' </div></div>'
            mouseoverText += '<div style="width:100%;display:flex;"><div style="flex:0 0 60px;">主导功能:</div><div style="flex:1 1 auto;"> '+parkFunctionDom+' </div></div>'
          }
          if(item.type === 'technology'){
            const {name, build_time} = item
            const for_new_link = item.for_new_link
            let for_new_linkDom = ''

            if(for_new_link){
              const primaryArr = for_new_link.split(',')
              primaryArr.map(item=>{
                for_new_linkDom += '<span style="display:block;color:#00FF33;">'+item+'</span>'
              })
            }
            mouseoverText += '<div style="color:#00FF33;font-weight: bold;">'+name+'</div>'
            mouseoverText += '<div style="width:100%;display:flex;"><div style="flex:0 0 60px;">成立时间:</div><div style="flex:1 1 auto;color:#00FF33;"> '+build_time+' </div></div>'
            mouseoverText += '<div style="width:100%;display:flex;"><div style="flex:0 0 60px;">所属创新链环节:</div><div style="flex:1 1 auto;"> '+for_new_linkDom+' </div></div>'
          }
          const myCompOverlay = new ComplexCustomOverlay(new BMap.Point(cp.lng,cp.lat),'',mouseoverText,type,id,mapArea);
