...
const singleIp =/^((?:(?:25[0-5]|2[0-4]\d|[1-9]\d{0,2}|0)\.){3}(?:25[0-5]|2[0-4]\d|[1-9]\d{0,2}|0))$/
...
//ip/掩码格式
const ipMask = /^((?:(?:25[0-5]|2[0-4]\d|[1-9]\d{0,2}|0)\.){3}(?:25[0-5]|2[0-4]\d|[1-9]\d{0,2}|0))\/((?:(?:25[0-5]|2[0-4]\d|[1-9]\d{0,2}|0)\.){3}(?:25[0-5]|2[0-4]\d|[1-9]\d{0,2}|0))$/
//单个端口
...
//手机号
const phoneNumber = /^1([38]\d|5[0-35-9]|7[3678])\d{8}$/
//域名
const domainName = /^(?=^.{3,255}$)[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+$/

//tcp/udpPort
const tcp_udp_port = /^(?:tcp|udp)\s([0-9]|[1-9]\d{1,3}|[1-5]\d{4}|6[0-4]\d{4}|65[0-4]\d{2}|655[0-2]\d|6553[0-5])$/

//字母数字下划线并且不能为纯数字
const not_all_count = /^\w*[a-zA-Z_]\w*$/
...
export { singleIp, rangeIp, groupIp, ipMask, singlePort, forName, forNumber, rangePort, forEmail, domainName, phoneNumber, tcp_udp_port, not_all_count }
